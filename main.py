...
telegram_channels = {
    ...
    #1320796214: 'https://t.me/proved_is_here',
    1547632806: 'https://t.me/CentrVED',
    #1203560567: 'https://t.me/markettwits',  # Канал аггрегатор новостей
}

rss_channels = {
    'www.customs.ru/federal': 'https://customs.gov.ru/press/federal/rss',
    'www.customs.ru/regional': 'https://customs.gov.ru/press/regional/rss',
     'www.rbc/rss': 'https://rssexport.rbc.ru/rbcnews/news/20/full.rss',
    'www.ria.ru/rss': 'https://ria.ru/export/rss2/archive/index.xml',
    #'www.1prime.ru': 'https://1prime.ru/export/rss2/index.xml',
    #'www.interfax/rss.ru': 'https://www.interfax.ru/rss.asp',
    #'www.kgd.gov.kz':  'https://kgd.gov.kz/ru/rss.xml',
    #'www.drevlit.ru':  'https://drevlit.ru/rss.xml',
}
...
def check_pattern_func(text):
    ...
    key_words = [
        'ФТС',
        'тамож',     # газпром
        'экспорт',     # газопровод, газофикация...
        'импорт',    # сервеный поток, северный поток 2, южный поток
        'пошлин',      # спг - сжиженный природный газ
        'контробанд',
        'ВЭД',
        'ЕАЭС',
        #'санкц',
...
# Телеграм парсер
client = telegram_parser('gazpcbil', api_id, api_hash, telegram_channels, posted_q,
                         n_test_chars, check_pattern_func, send_message_func,
                         tele_logger, loop)


# Список из уже опубликованных постов, чтобы их не дублировать
history = loop.run_until_complete(get_history(client, gazp_chat_id, n_test_chars, amount_messages))
...
for source, rss_link in rss_channels.items():
    ...
    async def wrapper(source, rss_link):
        try:
            ...
            #await send_error_message(message,  gazp_chat_id, logger)
